...
* mvn assembly:assembly - Generate one uber-JAR with all dependencies in the JAR. This is useful running the CLI interface.
...
You will need to have network connectivity for the integration test to work. The API credentials currently used are specific to the sample testing. Do not use them for your own account.
...
A small CLI application is provided to help you run the API on the command line. Assuming you have generated a JAR with all dependencies (and you just don't like using curl on the command line), you can run the CLI as follows:

==== Serializing files to the CLI

It may be cumbersome to write JSON into your command line. In this case, you can serialize your files straight to the CLI by running the following Unix commands:

more your_file.json | xargs ./cli.sh -cd

This will send the contents of your_file.json into the CLI to create a directing job.
